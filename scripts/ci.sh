...
for PLATFORM in "${PLATFORMS[@]}"; do
    ...
    if [[ "${ENABLE_SLIM}" == "true" && "${TARGET}" == "run" && ${_ENABLE_IMAGE_PUSH} == "true" ]]; then
        ...
        docker pull "${image}"
        IFS=' ' read -ra _slim_args <<< "${SLIM_BUILD_ARGS}"
        IFS=' ' read -ra _additional_slim_args <<< "${ADDITIONAL_SLIM_BUILD_ARGS}"
        ./mint slim --target "${image}" --tag "${slim_image}" "${_slim_args[@]}" "${_additional_slim_args[@]}"
